    ...
    (if (and (null? consts) (null? blocks))
        ...
        (if (or (null? nums) (< (car nums) 0))
            ...
            (let* ((labblock (newlabel))
              ...
              (for-each (lambda (c)
                          (let* ((lab (newlabel)))
                            ...
                            (bytecode-put-u32-le (switch-const-tag i))
                            (bytecode-emit-labref lab)
                            (bytecode-put-u32-le POP)
                            (bytecode-put-u32-le 1)
                            (compile-expr env stacksize istail (switch-const-rhs e))
            ...
            (let* ((numconsts (car nums))
              ...
              (for-each (lambda (i)
                          (let* ((p (find (match-lambda (lab . c) (= i (switch-const-tag c)))) constslabs))
                                 (lab (if (pair? p) (car p) defaultlab))
                            (bytecode-emit-labref-with-pos lab basepos))
                          ) (range 0 numconsts))
              (for-each (lambda (i)
                          (let* ((p (find (match-lambda ((lab . b) (= i (switch-block-tag n)))) blockslabs))
                                 (lab (if (pair? p) (car p) defaultlab)))
                            (bytecode-emit-labref-with-pos lab basepos))
                          ) (range 0 numblocks))
...
(define (compile-expr env stacksize istail expr)
  ...
  (cond ((equal? (car expr) 'EVar)
         (access-var (var-get-location (env-get-var env (car (cdr expr)))) stacksize))
        ...
        ((equal? (car expr) 'ERecordwith)
         (let* ((e (car (cdr expr)))
                 (l (car (cdr (cdr expr))))
                 (size (field-get-numfields (env-get-field env (car (car l)))))
                 (lf (map (lambda (fe)
                            (match-let ((($ <field> index numfields) (env-get-field env (car fe))))
                              (assert (= numfields size))
                              (cons index (cdr fe)))) l)))
        ...
        ((equal? (car expr) 'EApply)
         (let* ((f (car (cdr expr)))
                ...
                (f-location (var-get-location vf))
                (f-shape (var-get-funshape vf))
...
(define (expr-fv expr env)
  (cond ((equal? (car expr) 'EVar)
         (let* ((v (car (cdr expr)))
                (loc (var-get-location (env-get-var env v))))
