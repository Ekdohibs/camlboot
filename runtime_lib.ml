...
let prim1 f wrap_exn unwrap1 wrap =
  Prim (fun x -> wrap (builtin_exn_handler wrap_exn (fun () -> f (unwrap1 x))))

let prim2 f wrap_exn unwrap1 unwrap2 wrap =
  Prim (fun x -> prim1 (f (unwrap1 x)) wrap_exn unwrap2 wrap)

let prim3 f wrap_exn unwrap1 unwrap2 unwrap3 wrap =
  Prim (fun x -> prim2 (f (unwrap1 x)) wrap_exn unwrap2 unwrap3 wrap)

let prim4 f wrap_exn unwrap1 unwrap2 unwrap3 unwrap4 wrap =
  Prim (fun x -> prim3 (f (unwrap1 x)) wrap_exn unwrap2 unwrap3 unwrap4 wrap)
